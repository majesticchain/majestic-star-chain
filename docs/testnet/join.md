...
| ---------------- | --------------------------------- | ------------------------------------------------------------------------ | ------------------------------------------------------------------------- |
| `evmos_9000-2`   | Olympus Mons Incentivized Testnet | [Olympus Mons](https://github.com/tharsis/testnets/tree/main/arsia_mons) | [`v0.3.x`](https://github.com/majesticchain/majestic-star-chain/releases) |
| `evmos_9000-1`   | Arsia Mons Testnet                | [Arsia Mons](https://github.com/tharsis/testnets/tree/main/arsia_mons)   | [`v0.1.x`](https://github.com/majesticchain/majestic-star-chain/releases) |
...
```bash
mjtd config chain-id evmos_9000-2
```
...
```bash
mjtd init <your_custom_moniker> --chain-id evmos_9000-2
```
...
By default, the `init` command creates your `~/.mjtd` (i.e `$HOME`) directory with subfolders `config/` and `data/`.
...
Check the `genesis.json` file from the [`testnets`](https://github.com/tharsis/testnets) repository and copy it over to the `config` directory: `~/.mjtd/config/genesis.json`. This is a genesis file with the chain-id and genesis accounts balances.
...
Your node needs to know how to find [peers](https://docs.tendermint.com/master/tendermint-core/using-tendermint.html#peers). You'll need to add healthy [seed nodes](https://docs.tendermint.com/master/tendermint-core/using-tendermint.html#seed) to `$HOME/.mjtd/config/config.toml`. The [`testnets`](https://github.com/tharsis/testnets) repo contains links to some seed nodes.

Edit the file located in `~/.mjtd/config/config.toml` and the `seeds` to the following:
...
sed -i.bak -e "s/^seeds =.*/seeds = \"$SEEDS\"/" ~/.mjtd/config/config.toml
```
...
We can set the [`persistent_peers`](https://docs.tendermint.com/master/tendermint-core/using-tendermint.html#persistent-peer) field in `~/.mjtd/config/config.toml` to specify peers that your node will maintain persistent connections with. You can retrieve them from the list of
...
```bash
sed -i.bak -e "s/^persistent_peers *=.*/persistent_peers = \"$PEERS\"/" ~/.mjtd/config/config.toml
```
...
```bash
rm $HOME/.mjtd/config/addrbook.json $HOME/.mjtd/config/genesis.json
