...
First, stop your instance of `mjtd`. Next, upgrade the software:
...
You will need to ensure that the version installed matches the one needed for th testnet. Check the Evmos [release page](https://github.com/majesticchain/majestic-star-chain/releases) for details on each release.
...
If the software version does not match, then please check your $PATH to ensure the correct mjtd is running.
...
echo "export DAEMON_NAME=mjtd" >> ~/.profile
echo "export DAEMON_HOME=$HOME/.mjtd" >> ~/.profile
echo 'export PATH="$DAEMON_HOME/cosmovisor/current/bin:$PATH"' >> ~/.profile
source ~/.profile
```

After this, you must make the necessary folders for cosmosvisor in your daemon home directory (~/.mjtd).

```bash
mkdir -p ~/.mjtd/cosmovisor/upgrades
mkdir -p ~/.mjtd/cosmovisor/genesis/bin
cp $(which mjtd) ~/.mjtd/cosmovisor/genesis/bin/

# Verify the setup
# It should return the same version as mjtd
cosmovisor version
```
...
```bash
$ curl https://raw.githubusercontent.com/tharsis/testnets/main/olympus_mons/genesis.json > ~/.mjtd/config/genesis.json
```
...
There shouldn't be any chain database yet, but in case there is for some reason, you should reset it. This is a good idea especially if you ran `mjtd start` on an old, broken genesis file.

```bash
mjtd unsafe-reset-all
```

#### Ensure that you have set peers

In `~/.mjtd/config/config.toml` you can set your peers. See the [peers.txt](https://github.com/tharsis/testnets/blob/main/olympus_mons/peers.txt) file for a list of up to date peers.
...
```bash
sudo tee /etc/systemd/system/mjtd.service > /dev/null <<EOF
[Unit]
...
Environment="DAEMON_HOME=$HOME/.mjtd"
Environment="DAEMON_NAME=mjtd"
...
sudo -S systemctl enable mjtd
sudo -S systemctl start mjtd
```

You can check the status with:

```bash
systemctl status mjtd
```
...
To upgrade the genesis file, you can either fetch it from a trusted source or export it locally using the `mjtd export` command.
...
```bash
mjtd migrate [target-version] [/path/to/genesis.json] --chain-id=<new_chain_id> --genesis-time=<yyyy-mm-ddThh:mm:ssZ>
```
