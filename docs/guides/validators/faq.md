...
Evmos is a public Proof-of-Stake (PoS) blockchain, meaning that validator's weight is determined by the amount of staking tokens (Mjt) bonded as collateral. These staking tokens can be staked directly by the validator or delegated to them by Mjt holders.
...
Delegators are Mjt holders who cannot, or do not want to run validator operations themselves. Users can delegate Mjts to a validator and obtain a part of its revenue in exchange (for more detail on how revenue is distributed, see [What is the incentive to stake?](#what-is-the-incentive-to-stake) and [What is a validator's commission?](#what-is-a-validators-commission) sections below).
...
- **Minimum self-bond amount**: Minimum amount of Mjt the validator needs to have bonded at all times. If the validator's self-bonded stake falls below this limit, its entire staking pool will be unbonded.
- **Initial self-bond amount**: Initial amount of Mjt the validator wants to self-bond.
...
Once a validator is created and registered, Mjt holders can delegate Mjts to it, effectively adding stake to its pool. The total stake of a validator is the sum of the Mjt self-bonded by the validator's operator and the Mjt bonded by external delegators.
...
- `unbonded`: Validator is not in the active set, and therefore not signing blocks. Unbonded validators cannot be slashed, but do not earn any rewards from their operation. It is still possible to delegate Mjt to this validator. Un-delegating from an `unbonded` validator is immediate.

Delegators have the same state as their validator.

::: warning
Delegations are not necessarily bonded. Mjt can be delegated and bonded, delegated and unbonding, delegated and unbonded, or liquid.
:::

### What is "self-bond"? How can I increase my "self-bond"?

The validator operator's "self-bond" refers to the amount of Mjt stake delegated to itself. You can increase your self-bond by delegating more Mjt to your validator account.
...
### Is there a minimum amount of Mjt that must be staked to be an active (bonded) validator?
...
- **Amount of self-bonded Mjt:** Number of Mjts a validator self-bonded to its staking pool. A validator with higher amount of self-bonded Mjt has more skin in the game, making it more liable for its actions.

- **Amount of delegated Mjts:** Total number of Mjt delegated to a validator. A high stake shows that the community trusts this validator, but it also means that this validator is a bigger target for hackers. Validators are expected to become less and less attractive as their amount of delegated Mjt grows. Bigger validators also increase the centralization of the network.
...
Staking Mjt can be thought of as a safety deposit on validation activities. When a validator or a delegator wants to retrieve part or all of their deposit, they send an unbonding transaction. Then, Mjt undergo a _three weeks unbonding period_ during which they are liable to being slashed for potential misbehaviors committed by the validator before the unbonding process started.

Validators, and by association delegators, receive block provisions, block rewards, and fee rewards. If a validator misbehaves, a certain portion of its total stake is slashed (the severity of the penalty depends on the type of misbehavior). This means that every user that bonded Mjt to this validator gets penalized in proportion to its stake. Delegators are therefore incentivized to delegate to validators that they anticipate will function safely.

### Can a validator run away with its delegators' Mjt?

By delegating to a validator, a user delegates staking power. The more staking power a validator has, the more weight it has in the consensus and processes. This does not mean that the validator has custody of its delegators' Mjt. _By no means can a validator run away with its delegator's funds_.
...
### How often will a validator be chosen to propose the next block? Does it go up with the quantity of Mjt staked?

The validator that is selected to mine the next block is called the **proposer**, the "leader" in the consensus for the round. Each proposer is selected deterministically, and the frequency of being chosen is equal to the relative total stake (where total stake = self-bonded stake + delegators stake) of the validator. For example, if the total bonded stake across all validators is 100 Mjt, and a validator's total stake is 10 Mjt, then this validator will be chosen 10% of the time as the proposer.
...
- **Block rewards:** Native tokens of applications run by validators (e.g. Mjts on Evmos) are inflated to produce block provisions. These provisions exist to incentivize Mjt holders to bond their stake, as non-bonded Mjt will be diluted over time.
- **Transaction fees:** Evmos maintains a whitelist of token that are accepted as fee payment. The initial fee token is the `mjt`.
...
Block provisions (rewards) are distributed proportionally to all validators relative to their total stake (voting power). This means that even though each validator gains Mjts with each provision, all validators will still maintain equal weight.

Let us take an example where we have 10 validators with equal staking power and a commission rate of 1%. Let us also assume that the provision for a block is 1000 Mjts and that each validator has 20% of self-bonded Mjt. These tokens do not go directly to the proposer. Instead, they are evenly spread among validators. So now each validator's pool has 100 Mjts. These 100 Mjts will be distributed according to each participant's stake:

- Commission: `100*80%*1% = 0.8 Mjts`
- Validator gets: `100\*20% + Commission = 20.8 Mjts`
- All delegators get: `100\*80% - Commission = 79.2 Mjts`

Then, each delegator can claim its part of the 79.2 Mjts in proportion to their stake in the validator's staking pool. Note that the validator's commission is not applied on block provisions. Note that block rewards (paid in Mjts) are distributed according to the same mechanism.
...
Let's take a concrete example to illustrate the aforementioned concept. In this example, there are 10 validators with equal stake. Each of them applies a 1% commission and has 20% of self-bonded Mjt. Now comes a successful block that collects a total of 1005 Mjts in fees. Let's assume that the proposer included 100% of the signatures in its block. It thus obtains the full bonus of 5%.
...
- For the proposer validator:
  ...
  - The pool obtains $R ~ + ~ 5\%(R)$: 105 Mjts
  - Commission: $105 ~ * ~ 80\% ~ * ~ 1\%$ = 0.84 Mjts
  - Validator's reward: $105 ~ * ~ 20\% ~ + ~ Commission$ = 21.84 Mjts
  - Delegators' rewards: $105 ~ * ~ 80\% ~ - ~ Commission$ = 83.16 Mjts \(each delegator will be able to claim its portion of these rewards in proportion to their stake\)

  - The pool obtains $R$: 100 Mjts
  - Commission: $100 ~ * ~ 80\% ~ * ~ 1\%$ = 0.8 Mjts
  - Validator's reward: $100 ~ * ~ 20\% ~ + ~ Commission$ = 20.8 Mjts
  - Delegators' rewards: $100 ~ * ~ 80\% ~ - ~ Commission$ = 79.2 Mjts \(each delegator will be able to claim its portion of these rewards in proportion to their stake\)
...
### Do validators need to self-bond Mjts
...
Even though there is no obligation for validators to self-bond Mjt, delegators should want their validator to have self-bonded Mjt in their staking pool. In other words, validators should have skin-in-the-game.

In order for delegators to have some guarantee about how much skin-in-the-game their validator has, the latter can signal a minimum amount of self-bonded Mjt. If a validator's self-bond goes below the limit that it predefined, this validator and all of its delegators will unbond.
