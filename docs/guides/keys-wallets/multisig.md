...
```bash
mjtd keys add --multisig=name1,name2,name3[...] --multisig-threshold=K new_key_name
```
...
```bash
mjtd keys add --multisig=p1,p2,p3 --multisig-threshold=2 multisig_address
mjtd keys add --multisig=p2,p3,p1 --multisig-threshold=2 multisig_address
```

Multisig addresses can also be generated on-the-fly and printed through the which command:

```bash
mjtd keys show --multisig-threshold=K name1 name2 name3 [...]
```
...
Let's add 10 MJT to the multisig wallet:

```bash
mjtd tx send \
    test1 \
    evmos1e0fx0q9meawrcq7fmma9x60gk35lpr4xk3884m \
    10000000000000000000amjt \
...
We want to send 5 MJT from our multisig account to `evmos1rgjxswhuxhcrhmyxlval0qa70vxwvqn2e0srft`.

```bash
mjtd tx send \
    evmos1rgjxswhuxhcrhmyxlval0qa70vxwvqn2e0srft \
    evmos157g6rn6t6k5rl0dl57zha2wx72t633axqyvvwq \
    5000000000000000000amjt \
    --gas=200000 \
    --fees=1000000amjt \
    --chain-id=evmos_9000-2 \
...
```sh
mjtd tx broadcast signedTx.json \
    --chain-id=evmos_9000-2 \
    --broadcast-mode=block
```
