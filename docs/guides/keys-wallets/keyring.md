...
You can use `mjtd keys` for help with the keys command and `mjtd keys [command] --help` for more information about a particular subcommand.
...
```bash
mjtd keys add mykey --keyring-backend test

# Put the generated address in a variable for later use.
MY_VALIDATOR_ADDRESS=$(mjtd keys show mykey -a --keyring-backend test)
```
...
yes $KEYPASSWD | mjtd keys add me
yes $KEYPASSWD | mjtd keys show me
# start mjtd with keyring-backend flag
mjtd --keyring-backend=file start
```
