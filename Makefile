...
SIMAPP = ./app
HTTPS_GIT := https://github.com/majesticchain/majestic-star-chain.git
...
build-docker:
	...
	mkdir -p ./build/
	$(DOCKER) cp evmos:/usr/bin/mjtd ./build/
...
godocs:
	@echo "--> Wait a few seconds and visit http://localhost:6060/pkg/github.com/majesticchain/majestic-star-chain/types"
	godoc -http=:6060
...
format:
	...
	find . -name '*.go' -type f -not -path "./vendor*" -not -path "*.git*" -not -path "./client/docs/statik/statik.go" -not -name '*.pb.go' | xargs goimports -w -local github.com/majesticchain/majestic-star-chain
.PHONY: format
...
ifeq ($(OS),Windows_NT)
	...
	IF not exist "build/node0/$(EVMOS_BINARY)/config/genesis.json" docker run --rm -v $(CURDIR)/build\evmos\Z mjtd/node "./mjtd testnet --v 4 -o /evmos --keyring-backend=test --ip-addresses mjtdnode0,mjtdnode1,mjtdnode2,mjtdnode3"
	docker-compose up -d
else
	mkdir -p localnet-setup
	@$(MAKE) localnet-build

	if ! [ -f localnet-setup/node0/$(EVMOS_BINARY)/config/genesis.json ]; then docker run --rm -v $(CURDIR)/localnet-setup:/evmos:Z mjtd/node "./mjtd testnet --v 4 -o /evmos --keyring-backend=test --ip-addresses mjtdnode0,mjtdnode1,mjtdnode2,mjtdnode3"; fi
	docker-compose up -d
endif
...
localnet-unsafe-reset:
	docker-compose down
ifeq ($(OS),Windows_NT)
	@docker run --rm -v $(CURDIR)\localnet-setup\node0\mjtd:evmos\Z mjtd/node "./mjtd unsafe-reset-all --home=/evmos"
	@docker run --rm -v $(CURDIR)\localnet-setup\node1\mjtd:evmos\Z mjtd/node "./mjtd unsafe-reset-all --home=/evmos"
	@docker run --rm -v $(CURDIR)\localnet-setup\node2\mjtd:evmos\Z mjtd/node "./mjtd unsafe-reset-all --home=/evmos"
	@docker run --rm -v $(CURDIR)\localnet-setup\node3\mjtd:evmos\Z mjtd/node "./mjtd unsafe-reset-all --home=/evmos"
else
	@docker run --rm -v $(CURDIR)/localnet-setup/node0/mjtd:/evmos:Z mjtd/node "./mjtd unsafe-reset-all --home=/evmos"
	@docker run --rm -v $(CURDIR)/localnet-setup/node1/mjtd:/evmos:Z mjtd/node "./mjtd unsafe-reset-all --home=/evmos"
	@docker run --rm -v $(CURDIR)/localnet-setup/node2/mjtd:/evmos:Z mjtd/node "./mjtd unsafe-reset-all --home=/evmos"
	@docker run --rm -v $(CURDIR)/localnet-setup/node3/mjtd:/evmos:Z mjtd/node "./mjtd unsafe-reset-all --home=/evmos"
endif
...
PACKAGE_NAME:=github.com/majesticchain/majestic-star-chain
