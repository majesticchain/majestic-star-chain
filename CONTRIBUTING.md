...
1. Either [open](https://github.com/majesticchain/majestic-star-chain/issues/new/choose)
   or [find](https://github.com/majesticchain/majestic-star-chain/issues) an issue you have identified and would like to contribute to
   resolving.
...
some [good first issues](https://github.com/majesticchain/majestic-star-chain/issues?q=label%3A%22good+first+issue%22).

## <span id="adr">Architecture Decision Records (ADR)</span>

When proposing an architecture decision for Evmos, please create
an [ADR](https://github.com/majesticchain/majestic-star-chain/blob/main/docs/architecture/README.md) so further discussions can be
...
at `https://github.com/majesticchain/majestic-star-chain`, the code should never exist
at `$GOPATH/src/github.com/majesticchain/majestic-star-chain`. Instead, we use `git remote` to add the fork as a new remote for the
original repo,`$GOPATH/src/github.com/majesticchain/majestic-star-chain`, and do all the work there.

For instance, to create a fork and work on a branch of it, you would:

1. Create the fork on github, using the fork button.
2. Go to the original repo checked out locally. (i.e. `$GOPATH/src/github.com/majesticchain/majestic-star-chain`)
...
4. Create your feature branch from `development` either on `github.com/majesticchain/majestic-star-chain`, or your fork (
